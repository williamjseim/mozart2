...
namespace mozart2
{
    internal class Program
    {
        ...
        {
            string[] menuetsFiles = new string[176];
            string[] trioFiles = new string[96];
            for (int i = 0; i < 176; i++)
            {
                menuetsFiles[i] = "C:\\Users\\zbcwise\\source\\repos\\mozart\\Music\\M" + (1 + i) + ".wav";//file path for the menuets
            }
            for (int i = 0; i < 96; i++)
            {
                trioFiles[i] = "C:\\Users\\zbcwise\\source\\repos\\mozart\\Music\\T" + (1 + i) + ".wav";//file path for the trios 
            }
            ...
            for (int i = 0; i < 16; i++) 
            {
                int num = r.Next(2, 13);
                int num2 = r.Next(0, 6);
                Console.WriteLine(num + " " + num2 + " random"+ i+" for");
                Console.WriteLine(menuets[num - 2,i]+" Menuets");
                PlayMusic(menuets[num - 2,i], menuetsFiles);
                Console.WriteLine(trios[num2,i]+" Trios");
                PlayMusic(trios[num2,i], trioFiles);
            }
            
        }
        static void PlayMusic(int musicNumber, string[] musicFiles)//plays the music
        {
            System.Media.SoundPlayer sp = new System.Media.SoundPlayer();
            sp.SoundLocation = musicFiles[musicNumber];
            sp.LoadAsync();
            sp.PlaySync();
        }
    }
